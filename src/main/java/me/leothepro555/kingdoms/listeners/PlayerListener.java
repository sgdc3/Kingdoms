...
public class PlayerListener extends Listeners<Kingdoms> {
  ...
  public void onPlayerJoin(PlayerJoinEvent event) {
    ...
    if (plugin.isValidWorld(p.getWorld())) {
      if (plugin.getChunkKingdom(event.getPlayer().getLocation().getChunk()) != null) {
        if (!plugin.kingdoms.getKeys(false)
                            .contains(plugin.getChunkKingdom(event.getPlayer().getLocation().getChunk()))) {
          if (!plugin.getChunkKingdom(event.getPlayer().getLocation().getChunk()).equals("SafeZone") && !plugin
            ...
            p.sendMessage(ChatColor.AQUA + "Entering " + ChatColor.YELLOW + plugin
                    .getChunkKingdom(event.getPlayer().getLocation().getChunk()));
          }
        }
      } else {
        event.getPlayer().sendMessage(ChatColor.AQUA + "Entering unoccupied land");
