...
public class TurretManager extends Listeners<Kingdoms> {
        ...
        if (event.getPlayer().getItemInHand().getItemMeta() != null) {
          if (event.getPlayer().getItemInHand().getItemMeta().getDisplayName() != null) {
            if (event.getPlayer().getItemInHand().getItemMeta().getDisplayName()
                     .equals(ChatColor.AQUA + "Arrow Turret")) {
              if (event.getBlockFace() == BlockFace.UP && (event.getClickedBlock().getType() == Material.FENCE || event
                      .getClickedBlock().getType() == Material.ACACIA_FENCE || event.getClickedBlock()
                                                                                    .getType() == Material.BIRCH_FENCE || event
                      .getClickedBlock().getType() == Material.DARK_OAK_FENCE || event.getClickedBlock()
                                                                                      .getType() == Material.JUNGLE_FENCE || event
                      .getClickedBlock().getType() == Material.NETHER_FENCE || event.getClickedBlock()
                                                                                    .getType() == Material.SPRUCE_FENCE)) {
                              ...
                              if (e instanceof LivingEntity) {
                                if (e instanceof Player) {
                                  if (((Player) e).getGameMode() == GameMode.SURVIVAL || ((Player) e)
                                    ...
                                    if (!plugin.turrets.getString(locationToString(turret.getLocation()))
                                                       .equals(plugin.getKingdom((Player) e))) {
                                      fireArrow(origin, e);
                                      break;
                                    }
                                  }
                                  ...
                                  if (((Wolf) e).isTamed()) {
                                    ...
                                    if (!plugin.turrets.getString(locationToString(turret.getLocation()))
                                                       .equals(plugin.getKingdom(owner))) {
                                      fireArrow(origin, e);
                                      break;
                                    }
                                  } else {
  ...
  public Location stringToLocation(String key) {
    ...
    if (split.length == 6) {
      Location loc = new Location(Bukkit.getWorld(split[0]), Integer.parseInt(split[1]), Integer
              .parseInt(split[2]), Integer.parseInt(split[3]), 0, 0);
      return loc;
    } else {
      return null;
    }
  ...
  public String locationToString(Location loc) {
    try {
      String key = loc.getWorld().getName() + " , " + (int) loc.getX() + " , " + (int) loc.getY() + " , " + (int) loc
              .getZ() + " , " + 0 + " , " + 0;
      return key;
